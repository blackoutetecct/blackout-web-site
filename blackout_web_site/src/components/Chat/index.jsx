...
import CloseChatImage from "../../images/closeChat.png";
      ...
      <div className="chatbox">
        <div className="chatbox__support" ref={chatRef}>
          <div className="chatbox__header">
            ...
            {messages && messages.length > 0 && <button className="chatbox__close--header" onClick={tentaFinalizarChat}>
              <img src={CloseChatImage} alt="Fechar" />
            </button>}
          </div>
          <div className="chatbox__messages" ref={divMensagensRef}>
            {messages && <>
              {!endConversation ? messages.map((message, index) => (
                <Message
                  message={message.text}
                  sender={message.sender === userId ? "me" : "support"}
                  key={index}
                />
              )) : <div className="confirm_modal--messages">
              <p className="confirm_modal--messages text">Tem certeza que deseja finalizar a conversa?</p>
              <div className="confirm_modal--messages button-container">
                <button className="confirm_modal--messages button" onClick={cancelarFinalizacaoChat}>Cancelar</button>
                <button className="confirm_modal--messages button" onClick={confirmarFinalizacaotoChat}>Confirmar</button>
              </div>
            </div> }
              </>
              }
